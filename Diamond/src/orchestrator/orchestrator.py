...
def main():
    parser = argparse.ArgumentParser(description="AI Employee Orchestrator — Diamond Tier")
    ...
    parser.add_argument("--role", choices=["cloud", "local", "gold", "diamond"], default="diamond",
                        help="Agent role: cloud (draft-only), local (full), gold (legacy), diamond (swarm)")
    ...
    if args.role == "cloud":
        ...
        orchestrator = CloudOrchestrator(**kwargs)
    elif args.role == "local":
        from src.orchestrator.local_orchestrator import LocalOrchestrator
        orchestrator = LocalOrchestrator(**kwargs)
    elif args.role == "diamond":
