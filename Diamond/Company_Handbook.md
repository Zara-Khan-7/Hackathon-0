# Company Handbook – Rules for AI Employee (Diamond Tier)
...
### Specialized Agents
- **Sales Agent**: Handles EMAIL_, LINKEDIN_, SALESPOST_ tasks. Expert in professional communication, CRM-aware.
- **Finance Agent**: Handles ODOO_, PAYMENT_, AUDIT_ tasks. Expert in invoicing, payments, financial reporting.
- **Content Agent**: Handles FACEBOOK_, INSTAGRAM_, TWITTER_, SOCIAL_, WHATSAPP_ tasks. Expert in platform-specific content.
- **Security Agent**: Validates ERROR_, EXECUTE_ tasks. Scans ALL outgoing content for sensitive data.

### A2A Messaging
- Agents communicate via in-memory message bus (mock RabbitMQ/Redis)
- Message types: task_delegation, task_result, info_request, info_response, security_alert, broadcast
- Priority queuing: urgent > high > normal > low

### Self-Improving Loop
- OutcomeTracker records every task result (success/failure/partial/timeout)
- PromptOptimizer analyzes patterns and generates recommendations
- PerformanceMetrics scores each agent on reliability, speed, and volume
- Optimization runs every 100 completed tasks

### Security Scanning
- Security Agent scans all outgoing content for sensitive keywords (password, token, api_key, etc.)
- Failed scans flag the task for mandatory human approval
- Security alerts broadcast to all agents via A2A bus
...
22. `delegate_to_agent` — Route task to best specialized agent (Diamond)
23. `optimize_prompts` — Run self-improving optimization loop (Diamond)
24. `compliance_report` — Generate compliance audit report (Diamond)
25. `crm_sync` — Synchronize CRM contacts and deals (Diamond)
26. `crisis_response` — Handle security alerts and emergencies (Diamond)
