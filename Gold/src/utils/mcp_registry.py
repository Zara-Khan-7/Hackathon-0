"""Unified MCP server registry and caller.

Routes tool calls to the correct MCP server (email, odoo, social) via subprocess.
Wraps calls with retry + error handling for resilience.
...
def call_mcp(server_name: str, tool_name: str, arguments: dict = None, timeout: int = 30) -> dict:
    """Call a tool on a registered MCP server.

    Args:
        server_name: One of "email", "odoo", "social"
